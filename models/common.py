...
class AFusion(nn.Module):
    ...
    def __init__(self, c1, c2):
        super().__init__()
        self.up = Upsample(c1, c2, scale_factor=2)
...
class Convafpn(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        self.up = Upsample(c2,c1)

    def forward(self, x):
        input1,input2 = x
        out = input1+self.up(input2)
...
class ASFF_4(nn.Module):
    def __init__(self, inter_dim=512, level=0, channel=[64, 128, 256, 512]):
        ...
        self.level = level
        if self.level == 0:  #这是从上往下数，第一层,0表示64，所以0改成3，其他数字全部减一
            self.upsample8x0 = Upsample(channel[3], channel[0], scale_factor=8)
            self.upsample4x0 = Upsample(channel[2], channel[0], scale_factor=4)
            self.upsample2x0 = Upsample(channel[1], channel[0], scale_factor=2)
        elif self.level == 1:  #第二
    ...
    def forward(self,x):
        ...
        if self.level == 0:
            input1 = self.upsample2x0(input1)
            input2 = self.upsample4x0(input2)
            input3 = self.upsample8x0(input3)
        elif self.level == 1:
            input0 = self.downsample2x1(input0)
            input2 = self.upsample2x1(input2)
            input3 = self.upsample4x1(input3)
        elif self.level == 2:
            input0 = self.downsample4x2(input0)
            input1 = self.downsample2x2(input1)
            input3 = self.upsample2x2(input3)
        elif self.level == 3:
            input0 = self.downsample8x3(input0)
            input1 = self.downsample4x3(input1)
            input2 = self.downsample2x3(input2)
...
class LDepthWiseConv(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size=1, stride=1):
        # 这一行千万不要忘记
        super(LDepthWiseConv, self).__init__()

        hidden = int(out_channel * 0.5)  # hidden channels

        self.point_conv =  nn.Conv2d(in_channel, hidden, 1, 1, 0, bias=False ,groups=1) #jiangwei

        self.L_conv = nn.Sequential(
            nn.Conv2d(hidden, hidden, kernel_size, stride, (kernel_size - 1) // 2, groups=hidden,bias=False),
            nn.BatchNorm2d(hidden),
            nn.ReLU(inplace=True)
        )

    def forward(self, input):
        out1 = self.point_conv(input)
        out = self.L_conv(out1)
...
class C3L_conv(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = LDepthWiseConv(c1, c_,1,1)
        self.cv2 = LDepthWiseConv(c1, c_,1, 1)
        self.cv3 = LDepthWiseConv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(BottleneckL_conv(c_, c_, shortcut, g, e=1.0) for _ in range(n)))

    def forward(self, x):
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1))
class BottleneckL_conv(nn.Module):
    # Standard bottleneck
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = LDepthWiseConv(c1, c_, 1, 1)
        self.cv2 = LDepthWiseConv(c_, c2, 3, 1)
...
#                          x[...,size_tensor[2]//2:,size_tensor[3]//
...
class MHSAPLv31(nn.Module):
    def __init__(self, n_dims, width=14, height=14, heads=16, pos_emb=False,e=0.5):
        super(MHSAPLv31, self).__init__()
        n_dims = width  # hidden channels
        # print(width)
        width = 14
        self.heads = heads
        self.query = nn.Conv2d(n_dims, n_dims, kernel_size=1)
        self.key = nn.Conv2d(n_dims, n_dims, kernel_size=1)
        self.value = nn.Conv2d(n_dims, n_dims,kernel_size=3,padding=1)
        self.pos = pos_emb
        if self.pos:
    ...
    def forward(self, x):
        ...
        out1 = torch.matmul(self.softmax(v), energy)
        out = out + out1
...
class C3_MHSA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(MHSA(c_) for _ in range(n)))
...
class C3afpn(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.up = Upsample(c2, c1, scale_factor=2)

    def forward(self, x):
        input1,input2 = x
        out = input1 + self.up(input2)

        return self.cv3(torch.cat((self.m(self.cv1(out)), self.cv2(out)), 1))
